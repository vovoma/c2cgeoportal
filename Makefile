...
.PHONY: tests
tests: $(BUILD_DIR)/requirements.timestamp geoportal/tests/functional/test.ini $(BUILD_DIR)/db.timestamp
...
geoportal/c2cgeoportal_geoportal/scaffolds/create/package.json_tmpl: ngeo/package.json $(BUILD_DIR)/requirements.timestamp geoportal/c2cgeoportal_geoportal/scripts/import_ngeo_apps.py
	$(PRERULE_CMD)
	$(BUILD_DIR)/venv/bin/import-ngeo-apps --package _ $< $@

geoportal/c2cgeoportal_geoportal/scaffolds/update/CONST_create_template/: geoportal/c2cgeoportal_geoportal/scaffolds/create/package.json_tmpl
